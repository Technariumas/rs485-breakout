...
0x1100 -- new feature - watchdog timer always on, fix sleep times, add firmware
version input register
...
inline static void serialReaderDisable() { PORTA |= _BV(READER_ENABLE); }

inline static void serialDriverDisable() { PORTA &= ~_BV(DRIVER_ENABLE); }
...
void transceiver_txen(void) { serialDriverEnable(); }
...
static inline bool isGpioConfigRegisterSet() {
...
void gpioControll(uint8_t ctrl) {
  if (ctrl & _BV(0)) {
    PORTA |= _BV(GPIO0);
  } else {
    PORTA &= ~_BV(GPIO0);
  ...
  if (modbusIsFrameAvailable()) {
    switch (modbusGetFunctionCode()) {
      ...
      case fcPresetSingleRegister: {
        ...
        } else if (isGpioConfigRegisterSet()) {
          uint8_t ctrl = (uint8_t)(holdingRegisters.asStruct.pin_ctrl & 0x00FF);
          gpioControll(ctrl);
        }
      } break;
...
inline static void wdtInterruptEnable() { WDTCSR |= _BV(WDIE); }

inline static void wdtInterruptDisable() { WDTCSR &= ~_BV(WDIE); }

ISR(WDT_vect) {
  // Just wakeup. For some reason enabling WDT interrupt inside of ISR does not
  // work, so we do it in sleep loop
}
